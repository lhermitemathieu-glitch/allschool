...
10. **Construire la page `/register`** (inscription candidat via Supabase) puis rebrancher les CTAs de la page d'accueil publique, actuellement tous pointés vers `/login` faute de page d'inscription. Concerne : nav (« Créer un compte »), section de conversion sombre (« Créer mon espace candidat/école — c'est gratuit »). 🔴 — la page publique pousse à créer un compte mais le parcours n'existe pas.
...
> ✅ Fait en session 16 : suppression du code mort signup + recherche « formations Allschool » (item #5) · migration 041 de nettoyage des tables mortes + retrait import candidats (item #2) · fix complétion profil bloquée à 92 % (critère `loisirs` toujours raté) + migration 042 suppression colonne morte `loisirs` (centres d'intérêt consolidés dans `passions`) · autocomplétion ville (API gouv) sur les expériences pro · tri des offres par date de publication.
> ✅ Fait en session 17 : **nouvelle page d'accueil publique** (`/`) — `components/public/AccueilPublic.jsx`, sans authentification. Hero dynamique à 2 modes (entreprise → offres via `/api/alternance` ; école → formations via `/api/formations-lba`), géocodage API gouv, toggle pill, section de conversion sombre, stats, footer légal. Remplace l'ancienne landing « En construction ». Police Inter ajoutée. CTAs → `/login` en attendant `/register` (item #10). Reste à surveiller : perf (une grande ville sans mot-clé remonte des milliers de résultats, plafonnés à 60 à l'affichage — cf. item #7).

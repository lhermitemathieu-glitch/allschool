...
export default function PanelCandidatOffresArchivees() {
  ...
  return (
    <>
      <div className="topbar">
        <div>
          <div className="page-title">Offres archivées</div>
          <div className="page-sub">
            {loading ? 'Chargement…' : `${items.length} offre${items.length !== 1 ? 's' : ''} masquée${items.length !== 1 ? 's' : ''}`}
          </div>
        </div>
      </div>
              ...
              <div key={item.id} style={{ display: 'flex', overflow: 'hidden', borderRadius: 10, border: '1px solid var(--border)', background: '#fafafa', opacity: 0.8 }}>
                ...
                <div style={{ flex: 1, padding: '12px 16px', display: 'flex', gap: 12, alignItems: 'center' }}>
                  ...
                  <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
                    ...
                    <button className="btn-sm teal" style={{ fontSize: 11 }} onClick={() => restaurer(item.id)}>
